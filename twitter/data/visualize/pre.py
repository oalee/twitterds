...
def plot_users(dataframe):
    plot_variables = [
        ('tweets_count', 'followersCount'),
        ('tweets_count', 'friendsCount'),
        ('tweets_count', 'likes_count')
    ]

    fig, axs = plt.subplots(len(plot_variables), 1, figsize=(10, 6 * len(plot_variables)))
    ...
    for idx, (x_var, y_var) in enumerate(plot_variables):
        ...
        axs[idx].set_title(f'{x_var.replace("_", " ").capitalize()} vs {y_var.replace("_", " ").capitalize()}')
